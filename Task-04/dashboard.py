...
class Dashboard(QWidget):
    ...
    def toggle_column(self, column):
      if column in self.selected_columns:
        ...
        self.output_console.append(f"Column deselected: {column}")
      else:
        self.selected_columns.append(column)
        self.output_console.append(f"Column selected: {column}")


    def execute_search(self):
        term = self.query_input.text().strip()
        if not self.selected_columns:
           self.output_console.append("No columns selected.")
           return

        columns_sql = []
        for col in self.selected_columns:
            if col == "stars":
               columns_sql.append("CONCAT_WS(', ', star1, star2, star3) AS stars")
            else:
              columns_sql.append(col)
        columns_sql = ", ".join(columns_sql)

        query = f"SELECT {columns_sql} FROM movies"
        params = []
        ...
        if term:
           if self.search_mode == "genre":
              ...
              params.append(f"%{term}%")
                ...
                query += " WHERE director LIKE %s"
                params.append(f"%{term}%")
           elif self.search_mode == "actor":
                query += " WHERE star1 LIKE %s OR star2 LIKE %s OR star3 LIKE %s"
                params.extend([f"%{term}%"] * 3)
           elif self.search_mode == "year":
                query += " WHERE released_year = %s"
                params.append(term)
           elif self.search_mode == "rating":
                query += " WHERE imdb_rating >= %s"
                params.append(term)

        conn = get_connection()
        if not conn:
           self.output_console.append("Error: Could not connect to MySQL.")
           return
        ...
        cursor.close()
        conn.close()
        self.output_console.append(f"Query executed: {len(rows)} rows found.")
    ...
    def export_csv(self):
        ...
        self.output_console.append("Data exported to new.csv")
